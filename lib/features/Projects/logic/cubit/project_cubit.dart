...
class ProjectCubit extends Cubit<ProjectsState> {
  ...
  ProjectCubit({required this.projectsRepo}) : super(const ProjectsState.initial());
  ...
  Future<void> fetchAllProjects() async {
    ...
    result.when(
      success: (data) => emit(ProjectsState.loaded(data)),
      error: (error) => emit(ProjectsState.error(error.error ?? "Unknown Error")),
    );
  }
}
